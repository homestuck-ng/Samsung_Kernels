    ...
    switch (sources[0].type) {
        case AUDIO_PORT_TYPE_DEVICE: // Patch for audio capture or loopback
            ...
            if (sinks[0].type == AUDIO_PORT_TYPE_MIX) {
                ...
                if (input_source == AUDIO_SOURCE_CAMCORDER && sec_device_->multidevice_rec && bt_sco_on
                    && AudioExtn::audio_devices_cmp(device_types, AUDIO_DEVICE_IN_2MIC)) {
...
#if defined(SEC_PRODUCT_FEATURE_BLUETOOTH_SUPPORT_A2DP_OFFLOAD)
